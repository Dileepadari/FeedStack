...
`.github/workflows/ci.yml` builds and tests on every push to `master` and every
pull request.

It runs on **Temurin 8**, because `maven.compiler.source` and `target` are both
pinned to `1.8`. Building on whatever JDK a runner happens to ship would test a
configuration nobody deploys.

Only `reader-core`, `reader-web-common` and `reader-web` are in the default
reactor; the agent and the distribution modules sit behind the `prod` profile and
are not built here. The Android client is a separate Gradle project and is not
built by CI at all.

Surefire reports are uploaded as an artifact on success and failure both.

## The summariser

`SummarizationStrategy` shells out to `summarizer.py`, which calls the Groq API.
Two rules, both of which exist because they were once broken:

**Nothing the subprocess prints on stderr reaches the reader.** stdout is the
summary; stderr is logged. They used to be merged and the whole stream returned
as the summary, so a Python traceback carrying the absolute path of the script on
the server was rendered into the article a user was reading.

**A failure returns a fixed string, never process output.** The exit code is
checked and there is a 60 second timeout. If anything goes wrong the reader sees
`Summary unavailable.` and the detail goes to the log.

`summarizer.py` does not install anything. It used to create a virtualenv, run
`pip install -r requirements.txt`, and fall back to `sudo apt install
python3.10-venv`, all triggered by an HTTP request. If the dependency is missing
it now says so on stderr and exits non-zero.

To make the feature actually work:

```bash
pip install -r requirements.txt
cp reader-web/tokens.env.example reader-web/tokens.env   # then add a Groq key
```

Without both, summaries read `Summary unavailable.` and the app is otherwise fine.
...
`README-light.md` is generated by `scripts/build-light-readme.mjs`. Edit
`README.md`, run `node scripts/build-light-readme.mjs`, commit both. It exits
non-zero if a marker it needs is missing.

Screenshots live in `docs/screenshots/{dark,light}` at 1440x900, captured against
a live instance with three subscriptions. The theme is a server-side user
setting, so switching it for a capture is `POST /api/user` with `theme=dark` and
a reload, not a client toggle.

## Licensing

**GPL-2.0**, inherited from [Sismics Reader](https://github.com/sismics/reader).
See `COPYING`. This is copyleft: anything distributed that is built from this must
also be GPL-2.0, and the upstream copyright stays with the code.

The `API_KEY` in `ContentUrlStrategy.java` and the `pk_live_` string in an HTML
test fixture both come from upstream and are not this project's to rotate.
